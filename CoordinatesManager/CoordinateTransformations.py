...
def general_coordinates_transformation(coordinates_list, direction = "Camera2Galvo", **kwargs):
    """
    ...
    if direction == "Camera2Galvo":
        ...
        Transformed_coordinates_voltage = transform_coordinates(coordinates_list, transform_matrix_cam2galvo)        
        # Transform voltages to PMT image pixel positions
        Transformed_coordinates = transform_between_PMT_Galvo(Transformed_coordinates_voltage, "Galvo2PMT", kwargs['scanning_config'])
        ...
        galvo_voltage_coordinates = transform_between_PMT_Galvo(coordinates_list, "PMT2Galvo", kwargs['scanning_config'])
        # Load transformation
        transform_matrix_cam2galvo = load_transformation("galvo_transformation")
        ...
        Transformed_coordinates = transform_coordinates(galvo_voltage_coordinates, transform_matrix_galvo2cam)
...
def load_transformation(target):
    ...
    """
    # Load transformation
    transformation_filename = os.path.join(os.getcwd()+"\CoordinatesManager\Registration\{}".format(target))
    transform_matrix_flat = np.loadtxt(transformation_filename)
    # Reshape the text matrix into (2, 2, 2)
    transform_matrix = np.reshape(transform_matrix_flat, (transform_matrix_flat.shape[1], -1, 2))
...
def transform_between_PMT_Galvo(coordinates_list, direction, scanning_config):
    ...
    for each_coordinate in coordinates_list:
        ...
        if direction == "Galvo2PMT":
            ...
            (((each_coordinate + galvo_scanning_voltage_range)/(galvo_scanning_voltage_range*2) * PMT_image_pixel_number).astype(int))
    
        elif direction == "PMT2Galvo":
            Transformed_coordinates.append\
            (((each_coordinate/PMT_image_pixel_number)*galvo_scanning_voltage_range*2 - galvo_scanning_voltage_range).astype(int))
    
    return np.array(Transformed_coordinates)


if __name__ == '__main__':
    new_list_of_coordinates = general_coordinates_transformation(np.array([[251, 249], [100,100]]), 'PMT2Camera', scanning_config = [5, 500])
