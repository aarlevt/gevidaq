...
class CoordinatesWidgetUI(QWidget):
    ...
    def init_gui(self):
        ...
        self.stageRegisterButton.setStyleSheet("QPushButton {color:white;background-color: blue; border-style: outset;border-radius: 8px;border-width: 2px;font: bold 12px;padding: 6px}"
        ...
        self.galvosRegisterButton.setStyleSheet("QPushButton {color:white;background-color: blue; border-style: outset;border-radius: 8px;border-width: 2px;font: bold 12px;padding: 6px}"
                                            "QPushButton:pressed {color:blue;background-color: darkblue; border-style: outset;border-radius: 8px;border-width: 2px;font: bold 12px;padding: 6px}"
                                            "QPushButton:hover:!pressed {color:blue;background-color: lightblue; border-style: outset;border-radius: 8px;border-width: 2px;font: bold 12px;padding: 6px}"
                                            "QPushButton:disabled: {color:blue;background-color: lightred; border-style: outset;border-radius: 8px;border-width: 2px;font: bold 12px;padding: 6px}")
        
        self.galvosScanMaskButton = QPushButton('Scan mask')
        self.galvosScanMaskButton.setStyleSheet("QPushButton {color:white;background-color: green; border-style: outset;border-radius: 8px;border-width: 2px;font: bold 12px;padding: 6px}"
        ...
        self.galvosScanStartSwitch = MySwitch('STOP', 'red', 'START', 'maroon', width = 50)
        ...
        self.dmdConnectButton = MySwitch('Disconnect', 'red', 'Connect', 'maroon', width = 70)
        ...
        self.dmdRegistrationButton.setStyleSheet("QPushButton:disabled {color:white;background-color: grey; border-style: outset;border-radius: 8px;border-width: 2px;font: bold 12px;padding: 6px}"
        ...
        self.selection_view_640.getView().setLimits(xMin = 0, xMax = 2048, yMin = 0, yMax = 2048, minXRange = 2048, minYRange = 2048, maxXRange = 2048, maxYRange = 2048)
        ...
        self.selection_view_532.getView().setLimits(xMin = 0, xMax = 2048, yMin = 0, yMax = 2048, minXRange = 2048, minYRange = 2048, maxXRange = 2048, maxYRange = 2048)
        ...
        self.selection_view_488.getView().setLimits(xMin = 0, xMax = 2048, yMin = 0, yMax = 2048, minXRange = 2048, minYRange = 2048, maxXRange = 2048, maxYRange = 2048)
        ...
        self.selection_view_2p.getView().setLimits(xMin = 0, xMax = 2048, yMin = 0, yMax = 2048, minXRange = 2048, minYRange = 2048, maxXRange = 2048, maxYRange = 2048)
        ...
        self.movieProjectionView.getView().setLimits(xMin = 0, xMax = 2048, yMin = 0, yMax = 2048, minXRange = 2048, minYRange = 2048, maxXRange = 2048, maxYRange = 2048)
        ...
        self.maskGeneratorStackContainer.setLayout(self.maskGeneratorStackContainerLayout)
        ...
        self.maskGeneratorStackLayer1Layout.addWidget(self.snapWhiteImageButton, 0, 1, 1, 1)
        
        label  = QLabel('Selection mode:')
        label.setAlignment(Qt.AlignRight)
        self.maskGeneratorStackLayer1Layout.addWidget(label, 1, 0, 1, 1)
        self.maskGeneratorStackLayer1Layout.addWidget(self.selectionModeMenu, 1, 1, 1, 1)
        ...
        self.polygonSelectionControlContainer.setLayout(self.polygonSelectionControlLayout)
        ...
        self.polygonSelectionOptionsContainer.setLayout(self.polygonSelectionOptionsLayout)
        ...
        self.polygonSelectionLayout.addWidget(self.polygonSelectionControlContainer, 0, 0, 2, 1)
        self.polygonSelectionLayout.addWidget(self.polygonSelectionOptionsContainer, 0, 1)
        self.polygonSelectionLayout.addWidget(self.polygonSelectionThicknessContainer, 1, 1)
        ...
        self.freehandSelectionControlContainer.setLayout(self.freehandSelectionControlLayout)
        ...
        self.freehandSelectionOptionsContainer.setLayout(self.freehandSelectionOptionsLayout)
        ...
        self.freehandSelectionLayout.addWidget(self.freehandSelectionControlContainer, 0, 0, 2, 1)
        self.freehandSelectionLayout.addWidget(self.freehandSelectionOptionsContainer, 0, 1)
        self.freehandSelectionLayout.addWidget(self.freehandSelectionThicknessContainer, 1, 1)
        ...
        self.maskGeneratorStackLayer1Layout.addWidget(self.stackedSelectionOptionsContainer, 2, 0, 1, 2)
    ...
    def popup_window_preview_mask(self, e):
        ...
        mask_preview_popup_window.setImage(self.backend.mask[self.drawing_laser].transpose(), levels=[0,1])
    ...
    def add_polygon_roi_for_2p(self):
        ...
        for i in range(8):
            angle = delta_angle * i
            handles.append([x0 + radius_x*0.8*np.sin(angle), y0 + radius_y*0.8*np.cos(angle)])
        
        rectangle_roi = pg.PolyLineROI(handles, pen = view.pen,closed=True, movable=True, removable=True)
    ...
    def browse_files(self):
        ...
        """
        self.backend.loadFileName, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Select file', './CoordinateManager/Images/',"(*.png)")
    ...
    def browse_folders(self):
        ...
        """
        self.backend.loadFolderName = QtWidgets.QFileDialog.getExistingDirectory(self, 'Select folder', './CoordinateManager/Images/')
...
class DMDWidget():
    ...
    def read_transformations_from_file(self):    
        try:
            with open('CoordinateManager/Registration/transformation.txt', 'r') as json_file:
                self.dict_transformations = json.load(json_file)    
        except:
            self.ui_widget.normalOutputWritten('No transformation could be loaded from previous registration run.')
            return
...
#         self.ui_widget.update_buttons()
    ...
    def save_mask(self):
        if not (self.ui_widget.freehandSaveMaskButton.isChecked() and self.ui_widget.selectionMode == 'freehandMode' 
                or self.ui_widget.polygonSaveMaskButton.isChecked() and self.ui_widget.selectionMode == 'polygonMode'):
            return
        
        image = Image.fromarray((self.mask[self.ui_widget.drawing_laser]*255).astype(np.uint8))
    ...
    def galvos_scan_mask(self):
        ...
        tp_digital = np.dtype([('Waveform', bool, (reference_length,)), ('Sepcification', 'U20')])
        
        if len(self.ui_widget.roi_list_2p) == 0:
            self.ui_widget.normalOutputWritten('No region selected for 2p galvo scanning')
            return
        
        roi_handle_positions = self.ui_widget.roi_list_2p[0].getLocalHandlePositions()
        
        for idx, pos in enumerate(roi_handle_positions):
            roi_handle_positions[idx] = self.ui_widget.selection_view_2p.getView().mapToView(pos[1])
        ...
        if 'camera-galvos' in self.dict_transformations.keys():
            vertices_transformed = transform(vertices, self.dict_transformations['camera-galvos'])
        else:
        ...
        contourScanningSignal = ProcessImage.mask_to_contourScanning_DAQsignals(filled_mask, OriginalImage, scanning_voltage, points_per_contour, sampling_rate, repeats = 1)
        ...
        self.galvoThread.setWave_contourscan(sampling_rate, contourScanningSignal, points_per_contour)
    ...
    def createMask(self):
        ...
        self.mask_transformed[self.ui_widget.drawing_laser] = ProcessImage.CreateBinaryMaskFromRoiCoordinates(list_of_rois_transformed, \
                                                                   fill_contour = flag_fill_contour, \
                                                                   contour_thickness = contour_thickness, \
                                                                   invert_mask = flag_invert_mode)
        self.mask[self.ui_widget.drawing_laser] = ProcessImage.CreateBinaryMaskFromRoiCoordinates(list_of_rois, \
                                                       fill_contour = flag_fill_contour, \
                                                       contour_thickness = contour_thickness, \
                                                       invert_mask = flag_invert_mode)
    ...
    def dmd_register(self, lasers_to_register):
        ...
        self.DMD_Registration_thread = threading.Thread(target = lambda: self.dmd_register_func(lasers_to_register))
        self.DMD_Registration_thread.start()
        self.DMD_Registration_thread.sig_finished_registration.connect(self.registration_finished)
    ...
    def check_mask_format_valid(self, mask):
        if len(mask.shape) == 3:
            self.ui_widget.normalOutputWritten('Image is stack; make sure to load a binary image; for now max projection used')
        ...
        if mask.shape[0] == 1024 and mask.shape[1] == 768:
            ...
            self.ui_widget.normalOutputWritten('Image has shape '+str(mask.shape[0])+'x'+str(mask.shape[1])+'; should be 768x1024')
            return False, None
    ...
    def loadFile(self):
        ...
        if not valid:
            ...
            self.mask[self.ui_widget.drawing_laser] = self.mask_transformed[self.ui_widget.drawing_laser] = valid_mask
    ...
    def loadFolder(self):
        ...
        if self.ui_widget.invertMovieButton.isChecked():
            self.mask_transformed[self.ui_widget.drawing_laser] = 1 - self.mask_transformed[self.ui_widget.drawing_laser]
