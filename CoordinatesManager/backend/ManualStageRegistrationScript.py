...
class Coordinates:
    ...
    def print_coords(self):
        ...
        self.relative_coords[:, 0] = 2 * (
            np.asarray(self.coords)[:, 0] - self.coords[4][0]
        )
        self.relative_coords[:, 1] = 2 * (
            np.asarray(self.coords)[:, 1] - self.coords[4][1]
        )

        print(self.relative_coords)

        self.transformation = CoordinateTransformations.polynomial2DFit(
            self.relative_coords, np.asarray(self.pos), order=1
        )
...
if __name__ == "__main__":
    ...
    if True:
        ...
        transformation = CoordinateTransformations.polynomial2DFit(
            relative_coords, local_pos, order=1
        )
    ...
    if True:
        ...
        for i in range(len(local_pos_name)):
            ...
            fig.suptitle(
                "Images at different relative stage positions around the position (-5000 mV, -5000 mV) with one marked target cell",
                y=0.8,
            )
            ...
            axs[0, i].plot(
                ...
                (y - delta, y + delta, y + delta, y - delta, y - delta),
                color="r",
            )
    ...
    if False:
        ...
        if False:
            ...
            for i in range(len(transformations)):
                ...
                x = np.arange(
                    origin_x[i] - delta + offset, origin_x[i] + delta + offset, 1
                )
                y = np.arange(
                    origin_x[i] - delta + offset, origin_x[i] + delta + offset, 1
                )
                ...
                # y = Y.ravel()
                # c_x = np.ones((3,3))
                Z_x = np.polynomial.polynomial.polyval2d(
                    X, Y, transformations[i][:, 0, :]
                )
    ...
    if False:
        ...
        coord_1_x_transformed = int(
            np.polynomial.polynomial.polyval2d(
                coord_1[0], coord_1[1], transformations[2][:, 0, :]
            )
        )
        coord_1_y_transformed = int(
            np.polynomial.polynomial.polyval2d(
                coord_1[0], coord_1[1], transformations[2][:, 1, :]
            )
        )
        coord_1_transformed = np.array((coord_1_x_transformed, coord_1_y_transformed))

        coord_2_x_transformed = int(
            np.polynomial.polynomial.polyval2d(
                coord_2[0], coord_2[1], transformations[2][:, 0, :]
            )
        )
        coord_2_y_transformed = int(
            np.polynomial.polynomial.polyval2d(
                coord_2[0], coord_2[1], transformations[2][:, 1, :]
            )
        )
        ...
        voltage_image[
            skimage.draw.rectangle(
                (coord_1_transformed[0], coord_1_transformed[1]),
                (coord_2_transformed[0], coord_2_transformed[1]),
            )
        ] = 255
