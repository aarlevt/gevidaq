...
ALP_AVAIL_MEMORY = (
    2003  # 	ALP on-board sequence memory available for further sequence
)
...
ALP_DEV_DMDTYPE = (
    2021  # 	Select DMD type; only allowed for a new allocated ALP-3 device
)
...
ALP_DMD_POWER_FLOAT = (
    1  # power down, release micro mirrors from deflected state
)

ALP_PWM_LEVEL = (
    2063  # PWM pin duty-cycle as percentage: 0..100%; after AlpDevAlloc: 0%
)
...
ALP_SEQ_REPEAT = (
    2100  # 	Non-continuous display of a sequence (AlpProjStart) allows
)
...
ALP_FLEX_PWM = (
    3  # ALP_PWM_MODE: all bit planes of the sequence are displayed as
)
...
ALP_SCROLL_FROM_ROW = (
    2123  # 	combined value from ALP_FIRSTFRAME and ALP_FIRSTLINE
)
...
ALP_SEQ_DMD_LINES = (
    2125  # Area of Interest: Value = MAKELONG(StartRow, RowCount)
)
...
ALP_ILLUMINATE_TIME = (
    2204  # 	Duration of the display of one picture in microseconds
)
...
ALP_TRIGGER_IN_DELAY = (
    2207  # 	Delay of the start of picture display with respect to the
)
...
ALP_PROJ_SYNC = (
    2303  # 	Select from ALP_SYNCHRONOUS and ALP_ASYNCHRONOUS mode */
)
ALP_SYNCHRONOUS = (
    2304  # 	The calling program gets control back after completion */
)
# 	of sequence display. */
ALP_ASYNCHRONOUS = (
    2305  # 	The calling program gets control back immediatelly. */
)
...
ALP_PROJ_QUEUE_AVAIL = (
    2317  # number of available waiting positions in the queue */
)
...
ALP_PROJ_WAIT_PIC_TIME = (
    0  #  ALP_DEFAULT: AlpProjWait returns after picture time */
)
...
class ALP4:
    ...
    def Initialize(self, DeviceNum=None):
        ...
        self._checkError(
            self._ALPLib.AlpDevAlloc(
                DeviceNum, ALP_DEFAULT, ct.byref(self.ALP_ID)
            ),
            "Cannot open DMD.",
        )
        ...
        if (
            self.DMDType.value == ALP_DMDTYPE_XGA
            or self.DMDType.value == ALP_DMDTYPE_XGA_055A
            or self.DMDType.value == ALP_DMDTYPE_XGA_055X
            or self.DMDType.value == ALP_DMDTYPE_XGA_07A
        ):
            self.nSizeX = 1024
            self.nSizeY = 768
            ...
            self.DMDType.value == ALP_DMDTYPE_DISCONNECT
            or self.DMDType.value == ALP_DMDTYPE_1080P_095A
        ):
            self.nSizeX = 1920
            self.nSizeY = 1080
            ...
            self.DMDType.value == ALP_DMDTYPE_WQXGA_400MHZ_090A
            or self.DMDType.value == ALP_DMDTYPE_WQXGA_480MHZ_090A
        ):
            self.nSizeX = 2560
            self.nSizeY = 1600
        else:
    ...
    def ImgToBitPlane(self, imgArray, bitShift=0):
        ...
        for ind, value in enumerate(imgArray):
            bitPlane[(ind - ind % 8) // 8] += (2 ** (7 - ind % 8)) * (
                (int(value) >> bitShift) % 2
            )
        return bitPlane
    ...
    def DevControl(self, controlType, value):
        ...
        """
        self._checkError(
            self._ALPLib.AlpDevControl(
                self.ALP_ID, controlType, ct.c_long(value)
            ),
            "Error sending request.",
        )
    ...
    def DevControlEx(self, controlType, userStruct):
        ...
        self._checkError(
            self._ALPLib.AlpDevControlEx(
                self.ALP_ID, controlType, userStruct.byref()
            ),
            "Error sending request.",
        )
    ...
    def ProjControlEx(self, controlType, pointerToStruct):
        ...
        """
        self._checkError(
            self._ALPLib.AlpProjContro(
                self.ALP_ID, controlType, pointerToStruct
            ),
            "Error sending request.",
        )
    ...
    def FreeSeq(self, SequenceId=None):
        ...
        self.Seqs.remove(
            SequenceId
        )  # Removes the last SequenceId from sequence list
        self._checkError(
    ...
    def Halt(self):
        ...
        """
        self._checkError(
            self._ALPLib.AlpDevHalt(self.ALP_ID), "Cannot stop device."
        )
    ...
    def Free(self):
        ...
        """
        self._checkError(
            self._ALPLib.AlpDevFree(self.ALP_ID), "Cannot free device."
        )
        del self._ALPLib
