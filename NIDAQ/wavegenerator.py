...
class generate_digital_waveform():
    ...
    def generate(self):
        ...
        self.offsetsamples_number = int(1 + (self.waveoffset/1000)*self.Daq_sample_rate) # By default one 0 is added so that we have a rising edge at the beginning.
        self.offsetsamples = np.zeros(self.offsetsamples_number, dtype=bool) # Be default offsetsamples_number is an integer.
        
        self.sample_num_singleperiod = round(self.Daq_sample_rate/self.wavefrequency)
        self.true_sample_num_singleperiod = round((self.waveDC/100)*self.sample_num_singleperiod)
        self.false_sample_num_singleperiod = self.sample_num_singleperiod - self.true_sample_num_singleperiod
        
        self.sample_singleperiod = np.append(np.ones(self.true_sample_num_singleperiod, dtype=bool), np.zeros(self.false_sample_num_singleperiod, dtype=bool))
        self.repeatnumberintotal = int(self.wavefrequency*(self.waveperiod/1000))
        # In default, pulses * sample_singleperiod_2 = period
        self.sample_singlecycle = np.tile(self.sample_singleperiod, int(self.repeatnumberintotal)) # At least 1 rise and fall during one cycle
        
        if self.wavegap != 0:
            self.gapsample = np.zeros(self.wavegap, dtype=bool)
            self.waveallcyclewithgap = np.append(self.sample_singlecycle, self.gapsample)
        else:
...
class generate_ramp():
    ...
    def generate(self):
        ...
        t = np.linspace(0, (self.waveperiod/1000), int(self.Daq_sample_rate*(self.waveperiod/1000)))
